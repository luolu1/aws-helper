...
每台实例保留最近 10 次 IP 更换记录（`ip_history` 表），含各自存活时间、触发来源
（实例上报 / 面板手动）和原因。已换掉的条目存活时间定格在换掉那一刻，
不随当前时间增长；写新记录时顺带裁掉超出 10 条的旧记录。
...
实例页和 Lightsail 页还会把结果存进浏览器 `localStorage`，**进页面直接渲染快照，
不调 AWS** —— 刷新页面、重新登录、重开标签页、服务重启都是零调用，只有点
「刷新列表」或做完电源操作才真的回源。上面那层 10 秒进程内缓存挡不住 F5
（隔 10 秒以上刷新就是一次真实调用），也挡不住服务重启。

快照按 (账号, 区域) 分键，最多 12 份，超了丢最旧的。页面会标出快照年龄
（「本地快照，数据取于 8 分钟前」），不假装是刚拉的。
...
aws_helper/
  ...
  core/launch_deploy.py 开机时内联部署 autoip / DDNS
