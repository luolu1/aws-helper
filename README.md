...
AWS 专用管理面板，覆盖 **EC2 / Lightsail / Bedrock** 三类服务。

现有的多云面板里 AWS 基本都是"顺带支持"：能列实例但注入不了开机脚本，能开机但换不了 IP，
更没有 IP 被墙后自动更换。这个工具只做 AWS，把这几件事做完整。

从创建到运维一条线：一键开机、开机脚本与 BBR 加速、记录实例登录凭据、
换 IP 与重装系统（换根卷保留实例 ID 和 IP）、终止时连带清理计费资源。
创建实例时可勾选顺带部署 DDNS 和实例侧自动换 IP 探测器，开机自动装好。

**自动换 IP 是实例自己判断被墙**：探测脚本装在实例上，从境内方向探测国内站点，
只有连续失败才上报面板换 IP —— 面板在海外，从海外探测判断不了被墙。
正常状态不上报，AWS 调用也按数据变化频率做了缓存，避免频繁调用触发风控。

数据存 **Postgres**，AWS 凭据、代理、私钥、实例密码都加密落盘。
两种一键部署都会自动装好并初始化数据库：
...
| 服务 | 定位 | 面板能做什么 |
|---|---|---|
| **EC2** | 按需虚拟机 | 一键开机、创建时部署 DDNS/自动换 IP/BBR、脚本模板编辑、换 IP、重装、终止清理 |
...
| 自动换 IP | 实例自己朝境内探测，被墙才上报，面板换 IP；面板不做无意义的海外探测 |
| 部署状态 | 自动换 IP 和 DDNS 都能在面板看部署状态，手动检测给出具体排查方向 |
| IP 更换记录 | 每台实例最近 10 次 IP 更换与各自存活时间，标出触发来源和原因 |
...
**自动换 IP 只有一种探测方式：实例自己探**

探测脚本装在实例上，从境内方向 TCP 连一个国内站点，连续失败达到阈值才上报面板换 IP。

面板侧探测（早期的 `local` 模式）**已经移除**。它探的是「从面板到实例的端口通不通」，
而墙是单向的：面板通常也在海外，从海外连实例一直是通的，被墙了照样探不出来。
用这个信号触发换 IP 是错的，还要为此周期性调 `DescribeInstances` —— 白担风控风险。
升级上来的老规则会在启动时自动迁成 agent 模式。

保留的保守设计：

- 实例侧连续失败要达到你设的阈值（默认 3 次）才上报
- 上报成功后实例暂停探测 120 秒，避免面板还在换 IP 时重复上报
- 面板侧换过 IP 后有 30 分钟冷却期，期间收到上报也不再换
...
**B. 给已有实例部署** —— 在自动换 IP 页点「开启实例侧探测」，在弹窗里选账号、
区域、运行中的实例，面板生成一段一键部署脚本，复制到实例上以 root 执行。
...
时，那台机器的网络状况不代表这台，上报会被拒绝（对外统一 401，不区分原因）。
重新生成脚本会换发凭证，**旧脚本立刻失效** —— 这是最常见的 401 原因：
点了两次「生成部署脚本」，实例上装的是第一份。以最后生成的那一份为准，
重新在实例上执行一遍即可。面板的「检测」会把这种被拒记录展示出来，
不再把「实例连上了但凭证过期」误报成「从未收到上报」。
...
| 凭证对不上 | 实例连上了面板，但上报被拒 —— 通常是又点了一次「生成部署脚本」 |
| 正常 | 心跳在宽限期内 |
...
每行还有「IP 记录」，展开看这台实例**最近 10 次** IP 更换：

```
IP           状态     存活时间        启用于                触发          原因
52.10.0.7    正在用   3 小时 12 分    2026/9/4 20:07:29    实例上报被墙   连不上 www.baidu.com:443
52.10.0.4    已更换   1 天 4 小时     2026/9/3 15:55:02    实例上报被墙   连不上 www.baidu.com:443
52.10.0.2    已更换   6 小时 41 分    2026/9/3 09:14:33    面板手动       面板手动换 IP
```

存活时间是「这个 IP 从启用到被换掉之间用了多久」。已换掉的条目**时长定格**，
不会随当前时间一起涨 —— 只存启用时刻、由页面拿当前时间去减的话，
历史条目显示的就变成「多久以前开始用的」，那不是同一件事。当前正在用的那条
会持续增长。

触发来源分「实例上报被墙」和「面板手动」，原因带上实例上报时的具体内容，
所以能看出这个 IP 是被墙换掉的还是你自己点的。

每台实例只留 10 条，写入新记录时顺带裁掉更旧的 —— 无人值守跑几个月，
不裁剪会一直涨。删除账号时连带清空，避免实例 ID 被 AWS 复用后显示上一台的记录。
...
面板侧探测移除后，自动换 IP 在空闲时**完全不调 AWS** —— 没有轮询线程，
只有实例上报 blocked 时才调 `AllocateAddress` / `AssociateAddress` 那几个换 IP 接口。
换完立即作废实例列表缓存，否则页面会继续显示已失效的旧 IP。
...
| 位置 | 内容 |
|---|---|
| Postgres | AWS 账号（密文）、密钥对（密文）、代理（密文）、DDNS Token（密文）、实例登录密码（密文）、脚本模板、换 IP 规则、IP 更换历史、会话、日志 |
...
aws_helper/
  ...
  autoip.py           处理实例上报并换 IP（面板侧探测已移除）
  tasks.py            后台任务与进度跟踪
